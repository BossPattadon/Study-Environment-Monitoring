...
export default function HistoryPage() {
  ...
  const normalizedWeights = useMemo(() => {
    ...
    const total = w.light + w.noise + w.temperature + w.humidity + w.air;
    if (total === 0) return { light: 0, noise: 0, temperature: 0, humidity: 0, air: 0 };
    return {
      light:       Math.round((w.light       / total) * 100),
      noise:       Math.round((w.noise       / total) * 100),
      temperature: Math.round((w.temperature / total) * 100),
      humidity:    Math.round((w.humidity    / total) * 100),
      air:         Math.round((w.air         / total) * 100),
    };
  }, [settings.weights]);

  const subScoreChartData = useMemo(() => {
    const nw = normalizedWeights;

    const weightedTotal = stats.chronological.map((d) => {
      const scores = [
        d.avg_light_score,
        d.avg_noise_score,
        d.avg_temp_score,
        d.avg_humidity_score,
        d.avg_aqi_score,
      ];
      ...
      return parseFloat((
        ((d.avg_light_score    ?? 0) * w.light       +
         (d.avg_noise_score    ?? 0) * w.noise       +
         (d.avg_temp_score     ?? 0) * w.temperature +
         (d.avg_humidity_score ?? 0) * w.humidity    +
         (d.avg_aqi_score      ?? 0) * w.air)
        / total
      ).toFixed(2));
    });
    ...
    return {
      ...
      datasets: [
        ...
        },
        {
          ...
          fill: false,
          borderWidth: 1.5,
          borderDash: nw.light >= 25 ? [] : [4, 3],
        },
        {
          label: `Noise (${nw.noise}%)`,
          data: stats.chronological.map((d) => d.avg_noise_score ?? null),
          borderColor: "rgb(168, 85, 247)",
          tension: 0.25,
          spanGaps: true,
          fill: false,
          borderWidth: 1.5,
          borderDash: nw.noise >= 25 ? [] : [4, 3],
        },
        {
          label: `Temp (${nw.temperature}%)`,
          data: stats.chronological.map((d) => d.avg_temp_score ?? null),
          borderColor: "rgb(244, 63, 94)",
          tension: 0.25,
          spanGaps: true,
          fill: false,
          borderWidth: 1.5,
          borderDash: nw.temperature >= 25 ? [] : [4, 3],
        },
        {
          label: `AQI (${nw.air}%)`,
          data: stats.chronological.map((d) => d.avg_aqi_score ?? null),
          ...
          fill: false,
          borderWidth: 1.5,
          borderDash: nw.air >= 25 ? [] : [4, 3],
        },
        {
          label: `Humidity (${nw.humidity}%)`,
          data: stats.chronological.map((d) => d.avg_humidity_score ?? null),
          borderColor: "rgb(100, 116, 139)",
          tension: 0.25,
          spanGaps: true,
          fill: false,
          borderWidth: 1.5,
          borderDash: nw.humidity >= 25 ? [] : [4, 3],
        },
      ],
    };
  }, [stats.chronological, normalizedWeights, settings.weights]);

  const subScoreChartOptions: ChartOptions<"line"> = useMemo(
    () => ({
      responsive: true,
      maintainAspectRatio: false,
      interaction: { mode: "index", intersect: false },
      plugins: {
        ...
        title: {
          display: true,
          text: "Daily sub-score breakdown — weights from Settings",
          color: "#71717a",
          font: { size: 13, weight: "bold" },
        },
      },
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="mb-6 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950/60">
        {loading ? (
          ...
          <div className="h-72">
            <Line data={subScoreChartData} options={subScoreChartOptions} />
          </div>
        ) : (
          <div className="flex h-72 items-center justify-center text-sm text-zinc-500">
            No sub-score data yet.
          </div>
        )}
      </section>
